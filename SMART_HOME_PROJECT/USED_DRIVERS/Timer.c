...
/* s32Delay  in ms */ 
void Timer0_vidSetDelay(u32 u32Delay)
{

	#if (Timer0_Mode_Selector == Timer_Mode_Normal_Msk )
	//volatile to avoid the optimization 
		volatile u8 u8PreLoad = 0;
...
void PWM0_vidGenerate(u8 u8DutyCycle)
{
	//volatile to avoid the optimization
	volatile u8 u8data;
...
void Timer2_vidSetDelay(u32 u32Delay)
{
...
void PWM2_vidGenerate(u8 u8DutyCycle)
{
...
void Timer1_vidSetDely(u32 u32Delay)
{

	#if (Timer1_Mode_Selector == Timer1_Mode_Normal_Msk )
	//volatile to avoid the optimization
	volatile u16 u16PreLoad = 0;
	...
	else
	{
		...
		volatile u16 u16Count = 0; //volatile to avoid the optimization
...
void PWM1_vidGenerate(u8 u8DutyCycle)

{
	volatile u16 u16Data=0;
